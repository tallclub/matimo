...
Zero Node.js dependency. Dual-file convention:
  definition.yaml  code: './search.ts'   ← TS SDK reads this
  search.ts        TypeScript impl
  search.py        Python impl  ← Python SDK auto-resolves .ts → .py
...
class FunctionExecutor:
    ...
    async def execute(
        ...
        """
        Resolve, import, and call the Python run() function.

        The executor resolves relative paths against the tool's definition_path
        directory first, then falls back to self._base_path.
        """
        ...
        if not py_path.exists():
            raise MatimoError(
                ...
                f"Dual-file convention: create {py_path.name} alongside {exec_cfg.code}",
                ErrorCode.FILE_NOT_FOUND,
                {"tool_name": tool.name, "expected_path": str(py_path)},
            )
        ...
        try:
            if inspect.iscoroutinefunction(run_fn):
                ...
                result = await asyncio.wait_for(
                    asyncio.get_event_loop().run_in_executor(None, run_fn, call_params),
                    timeout=timeout_s,
                )
    ...
    def _resolve_python_path(self, tool: ToolDefinition) -> Path:
        """
        Resolve the .py file path from the tool's 'code' field.
        Applies .ts → .py substitution (dual-file convention).
        """
        exec_cfg = tool.execution
        ...
        # Auto-substitute .ts → .py (dual-file convention)
        if resolved.suffix == ".ts":
            py_candidate = resolved.with_suffix(".py")
            return py_candidate

        return resolved

    def _load_run_function(self, py_path: Path, tool_name: str) -> Any:
        """
