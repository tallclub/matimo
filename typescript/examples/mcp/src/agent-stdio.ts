 ...
 * ============================================================================
 *
 * PATTERN: True AI Agent with OpenAI + LangChain via MCP (stdio)
 * ─────────────────────────────────────────────────────────────────────────
 * Spawns `matimo mcp` as a subprocess (no server to start).
 ...
 *    TEST_CHANNEL=C0000000000   # optional: specific channel ID to use
 *
...
async function runMcpStdioAgent() {
  // Parse CLI arguments
  ...
  console.info('\n╔════════════════════════════════════════════════════════╗');
  console.info('║     Matimo MCP AI Agent - Stdio Transport               ║');
  ...
  console.info('🔌 Transport: stdio (spawns matimo mcp as subprocess)\n');

  console.info('🚀 Initializing Matimo MCP (stdio)...');
  ...
  try {
    ...
    if (tools.length === 0) {
      console.error('❌ No tools loaded. Check that @matimo/* packages are installed.');
      process.exit(1);
    }
    ...
    const userRequests = [
      ...
      },
      {
        ...
        request: `Send a message to channel ${activeChannel || 'the first available channel'} saying "Hello from Matimo MCP! 🚀 Testing stdio transport."`,
      },
      {
...
runMcpStdioAgent().catch(console.error);
